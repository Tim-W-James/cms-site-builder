...
const preview = async (
  ...
  const path = !queryPath
    ...
    : queryPath.map((pathSegment) => encodeURIComponent(pathSegment)).join("/");

  // Redirect to the path from the fetched post
  // We don't redirect to req.query.path as that might lead to open redirect
  // vulnerabilities, it must be URL encoded first.
